...
function useFilesTable(drive?: ArFSDrive) {
          ...
          <div className="flex-column white" style={{ gap: '5px' }}>
            ...
            <span style={{ fontSize: '12px', color: 'var(--text-subtle)' }}>
              {row.entityType === 'folder' ? (
                <>
                  {' '}
                  <FilesIcon width={12} height={12} fill="var(--text-subtle)" />
                  ({row.size})
                </>
              ) : (
                formatByteCount(row.size)
              )}
            </span>
          </div>
        ),
  ...
  async function generateRows(
    arfsEntities: Array<ArFSPublicFile | ArFSPublicFolder>,
  ) {
    setIsLoading(true);
    const fetchedRows = [];
  ...
  function mapMimeToIcon(type: string) {
    ...
    switch (mimeType) {
      ...
      default:
        <FileCodeIcon width={30} height={30} />;
    }
  }
