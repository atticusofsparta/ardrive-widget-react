...
customArweave
    ...
    const arweaveDataProvider = useArweaveCompositeDataProvider({customArweave: customArweave})
...
//onload
    ...
    async function getDrive({
        ...
        try {
            if (queryType === 'address' && !address || address &&  !ARNS_TXID_REGEX.test(address)) {
                throw Error('No address or arfs id provided')
            }
            if (queryType === 'arfsId' && !arfsId || arfsId && !ARFS_ID_REGEX.test(arfsId)) {
                throw Error('No address or arfs id provided')
            }
